 ...
 (function($) {
        ...
        $.fn.pp3Diso = function(params) {
	 	var params = $.extend( {
			...
			oncursordelay:function() {},
								// 
			onmoveavatar:function(){},			//--- après que l'avatar ait bougé
			beforemoveavatar:function(){ return true; },		// avant que l'avatar bouge
			onmovepathfinding:function(){},			//--- après chaque mouvement du pathfinding
			beforeclosewin:function() { return true; },	// avant de fermer le dialogue
			afterclosewin:function() {},			// après fermeture
			onenterbuilding:function(){},			// la souris survol un batiment
			onleavebuilding:function(){},			// la souris sort d'un batiment
			onenterobject:function(){},			// la souris entre un objet
			//onmoveobjet:function(x, y, mapId){},			// la souris survol un objet
			onleaveobject:function(){},			// la souris sort un objet
			onclicbuilding:function(){},	// clic sur un bâtiment
			onclicobject:function(){},	// clic sur un objet
			onchangezoom:function(){},			// lorsque le zoom est changé par la molette
			onclicright:'',
			}, params);
		...
		*/
		function first(map) {
			...
			if(!params.mode2d) {
				if(params.onclicright != '') {
					$('.pp3diso-shap').on('contextmenu', (function() {
						params.onclicright(avatar_posx, avatar_posy, params.mapId);
					}));
				}
				$('.pp3diso-shap').on('click', (function() {
				...
				if(params.cursorDelay !== false) {
					$('.pp3diso-shap').on('mouseenter', function() {
					...
					$('.pp3diso-shap').on('mouseout', function() {
						window.clearTimeout(cursorDelayTimer);
					});
				}
			...
			if(params.drag && !params.mode2d) {
				$('#pp3Diso-conteneur').on('touchstart', function(e){
					...
					$('#pp3Diso-conteneur').on('click', function() { return false; });
					$('#pp3Diso-conteneur').on('touchmove', function(e) { dragIOS(e); });
					
					$('#pp3Diso-conteneur').on('touchend touchcancel', function(e){
						dragStop(e, $(this)); 
					});
				...
				$('.pp3diso-shap').on('mousedown', function(e) {
						...
						$('#pp3Diso-conteneur').on('click', function() { return false; });
						$('#pp3Diso-conteneur').on('mousemove', dragAndDrop).on('mouseup', dragStop);
		...
		function dragStop(e, vis) {
			...
			$('#pp3Diso-conteneur').off('mousemove', dragAndDrop).off('mouseup', dragStop);
			
			//$(document).off('mousemove', dragAndDrop);
			//$(document).off('mouseup', dragStop);
			//$('#pp3Diso-conteneur').off('mousemove', pp3dISO_drag).off('mouseup', pp3Diso_drag_stop);
			if(old_padx != padx && old_pady != pady) {
				view(monde);
				//$('#infos').html(' Nbr=' + nbrDragStop);
				//nbrDragStop++;
			}
			$('.pp3diso-shap').off('touchmove');
			$('.pp3diso-shap').off('ontouchend');
			if(touchpad_flag) {
				touchpad_flag = false;
				//$('#infos').html('*ok*');
				evenement(vis);
		...
		function moveCursor(titleset) {
			...
			if(!params.mode2d) {
				...
				titleset.first('p').trigger('focus');
			}
		...
		function _interneCursor(on, off, decx, decy) {
			...
			if(!params.mode2d) {
				...
				$('#pp3diso-cursor-img').on('load', function() {
					if(this.width < 1) {
						//this.width = zoom;
						//this.height = zoom;
						//alert(zoom);
		...
		function _avatar(x, y, src, decx, decy, animation, ny) {
			...
			if(!params.mode2d) {
				...
				$('#pp3diso-avatar-img').on('load', function() {
					var imgx = this.width;
